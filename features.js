...
// ── HTML ESCAPING (local copy — forum.js's escHtml is private to its own IIFE and loads after this file) ──
...
function renderExamCountdown() {
  ...
  list.innerHTML = future.map(ex => {
    ...
    const color = diff < 0 ? 'var(--silver)' : diff === 0 ? '#ff5566' : diff <= 3 ? '#fbbf24' : diff <= 7 ? '#fb923c' : 'var(--teal)';
    const label = diff < 0 ? 'Past' : diff === 0 ? 'TODAY' : diff === 1 ? 'Tomorrow' : diff + ' days';
...
function exportJournalPDF() {
  ...
  const bodyHtml = entries.map(e => {
    ...
    const safeTitle = escHtml(e.title||'Untitled Entry');
    const safeBody = escHtml(e.body||'').replace(/\n/g,'<br>');
...
function exportFlashcardsPDF() {
  ...
  const rows = cards.map((c,i) => {
    const sf = escHtml(c.front||'');
    const sb = escHtml(c.back||'');
