...
20. **Mobile top-bar / bottom-"chin" safe-area bug — fixed and confirmed live on a real notched iPhone.** `#hdr` now pads for `env(safe-area-inset-top)`, `#app` uses `100dvh`, `#mobile-fab-list`/`#content`/`.tab` bottom spacing use `env(safe-area-inset-bottom)` instead of flat px. Also added a `controllerchange` listener in `index.html` that auto-reloads once a new service worker takes control — this was the actual reason shipped fixes kept looking "not applied" on the installed PWA (sw.js caches CSS cache-first; an already-open tab kept running old assets indefinitely without it).
...
21. **Custom install-as-PWA prompt.** Hooks into the existing exam/study-reminder notification bar (`showNotif`/`dismissNotif` in `features.js`) instead of duplicating it — `beforeinstallprompt` for Chrome/Edge/Android, manual "Add to Home Screen" instructions for iOS Safari (no programmatic install API there), skipped entirely when already running standalone, 14-day cooldown after dismissal.

22. **A 4th "Light" theme category** in Settings' theme picker (Vibes/Aesthetic/Wallpapers/**Light**) — 12 muted pastel tiles. Applying one swaps `--text`/`--text2`/`--silver` to a dark palette too (`applyBg()`, gated on a `light:true` flag per tile), since the default light-on-near-black text is unreadable on a light background — this was a real bug in the first pass, not just a nice-to-have.

23. **Settings restructure — Account and Themes moved out into their own reveal-on-demand tabs.** Settings used to nest 5 layers deep (sidebar → Account/Preferences groups → Look/Colors/Layout/Language sub-tabs → tile grid) and is now purely 4 flat Preferences tabs (AI Settings/AI Key/Layout/Language). Account (Details/Goals/Stats/Badges/Titles) and Themes (Master Theme/UI Skin/tile picker/Accent Color) are each their own full-page tab with no permanent sidebar entry — opening one (header avatar → Account, header palette icon → Themes) reveals a matching sidebar nav button with the same slide-in `openMusicPlayer()` already used for Music, and `go()` hides it again on navigating away. `go('profile')`/`go('account')` are now plain aliases, no more special-cased redirect into Settings.
    - **Real bug this exposed and fixed:** a hardcoded positional array (`settingsTabMap`) relabeled `.ps-tab` buttons by DOM index instead of by any per-button key, because these buttons have no `data-i18n` attribute of their own. Restructuring the tab order silently mislabeled buttons — looked exactly like a stale-cache issue (cost real time chasing the wrong hypothesis) until traced to this. Now split into two properly-scoped maps, one for `#tab-account`, one for `#tab-settings`.
    - **A second real bug from the same restructure:** making AI Settings the new default-shown panel in Settings meant it stopped being reached via an explicit `psGo('aiprofile', ...)` click, which used to be the only place `renderAIProfilePanel()` (binds every curriculum/year/subject chip's onclick) got called — so those chips were silently unclickable on first load. Fixed by calling it directly from `loadSettingsPage()`.

24. **AI Student Profile form (Settings → AI Settings) — attempted declutter, correctly called out as insufficient.** Added spacing between form groups and collapsed the Curriculum row behind a "Show all" toggle; caught and fixed the two real bugs above along the way. But per direct feedback this did **not** meaningfully declutter the form — it's still a lot of same-weight chip rows crammed on one screen (Curriculum, Year, 14+ Subjects, Explanation style, Biggest struggle, Study habit, Pressure level, Response language). **Don't consider this solved.** If picked up again: this probably needs an actual structural rethink (e.g. a multi-step wizard, grouped accordion sections, or moving less-essential fields like study-habit/pressure-level out of the primary view) rather than another spacing/collapse pass on the same flat layout.

## Digital Expo materials (separate from the app itself)

A large amount of work has also gone into **expo/pitch collateral**, all living in the same repo root as loose files (not part of the deployed site): `Veda-Expo-Deck.pptx`/`.html` (7-slide pitch deck, "Deep Ocean" palette — teal `#00D4AA` / blue `#38BDF8` on navy, distinct from the app's own `--accent`/`--teal`), `Veda-Foamboards.pdf`/`.html` (2× A1 portrait boards — now Arial Black/Outfit headings and labels, not Syne/DM Mono; icon strips color-matched to the tile grid below them), `Veda-Hero.html` (1:1 recreation of a landing hero mockup), `Veda-Badges.pdf` (9 named lanyard-badge squircles), `Veda-Brochure.pdf`, `Veda-Presentation-Script.docx`, `Veda Ticket.png.jpeg` (QR code to `contactus.veda-net.com` + "Arcade Token" label, Outfit font), `Veda-Team-Contact.html` (Gmail-compose links per teammate, also live at `contactus.veda-net.com` via a separate repo — `keshavandkarthik-cell/veda-contact` — with its own `CNAME`; DNS is on Cloudflare per the earlier email-routing screenshot), plus SDG tile images and various Muse-generated art assets. The title slide of the deck also now carries the MAZ InnoVision + MAZ International School logos (bottom-right, title slide only — not on content slides, kept off by request since this material gets reused for other competitions like the Sunway AI Cup and shouldn't be school/team-branded everywhere).

Foamboards/deck/ticket are built with standalone Node/Python generator scripts (pptxgenjs for the deck, inline HTML+CSS rendered to PDF via headless Chrome for the foamboards) that live only in ephemeral scratchpad dirs, not committed — if one needs regenerating, expect to rebuild the generator from scratch (assets can usually be regenerated: logo from `image_2.png`, SDG tiles from the `SDG*.png.jpeg` files in the repo root, QR via Python's `qrcode` package). This material is iterated on very frequently based on visual feedback (screenshots, "too small," "too plain," "add X") — treat it as design work, not engineering, and expect several fast visual-feedback loops per session. One recurring gotcha: AI-generated PNG assets from this feedback loop sometimes have transparency baked in as a visible checkerboard (not real alpha) — extracting real transparency needs a color/diff-threshold pass (see the doodle-graphic swap in the foamboards for the working approach), and a soft glow in the source art can make clean separation impossible, in which case dropping the glow and keeping just the crisp line art is the right call, not more threshold-tuning.
